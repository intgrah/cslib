...
@[scoped grind]
structure Candidate (S : Set (Term Var)) : Prop where
  lc {t} : t ∈ S → t.LC
  sn {t} : t ∈ S → SN Term.Red t
  red {t t'} : t ∈ S → t ⭢βᵛ t' → t' ∈ S
  neutral {t} : t.LC → Neutral t → t ∈ S
  headExpand {t} : t.LC → ¬ t.Value → (∀ t', t ⭢βᵛ t' → t' ∈ S) → t ∈ S
...
lemma maximalCand : Candidate (maximalSet (Var := Var)) := by
...
structure Valuation (Var : Type u) where
...
structure Valuation.IsValid (ρ : Valuation Var) : Prop where
  /-- Every free-variable assignment is a candidate. -/
  free_rc : ∀ X, Candidate (ρ.free X)
  /-- Every bound-variable assignment is a candidate. -/
  bound_rc : ∀ S ∈ ρ.bound, Candidate S

omit [DecidableEq Var] [HasFresh Var] in
/-- Pushing a candidate onto the bound list preserves validity. -/
lemma Valuation.IsValid.extend_bound {ρ : Valuation Var} {S : Set (Term Var)}
    (hρ : ρ.IsValid) (hS : Candidate S) : { ρ with bound := S :: ρ.bound }.IsValid where
  free_rc := hρ.free_rc
  bound_rc := by
    intro S' hS'
...
inductive SumInterp (A B : Set (Term Var)) : Term Var → Prop
  ...
  | red {t t' : Term Var} : SumInterp A B t → t ⭢βᵛ t' → SumInterp A B t'
  | headExpand {t : Term Var} : t.LC → ¬t.Value → (∀ t', t ⭢βᵛ t' → SumInterp A B t') →
      SumInterp A B t

/-- The Girard interpretation of a type as a reducibility candidate over a candidate environment. -/
def interp (ρ : Valuation Var) : Ty Var → Set (Term Var)
  | .top => maximalSet
  ...
  | .all σ τ =>
      { t | t.LC ∧ SN Term.Red t ∧
          ∀ S : Set (Term Var), Candidate S → (∀ s ∈ S, s ∈ interp ρ σ) →
            ∀ U : Ty Var, U.LC → Term.tapp t U ∈ interp { ρ with bound := S :: ρ.bound } τ }

/-- Every interpretation is a reducibility candidate, by structural induction on the type. -/
lemma interp_candidate {ρ : Valuation Var} (τ : Ty Var) (hρ : ρ.IsValid) :
    Candidate (interp ρ τ) := by
  induction τ generalizing ρ with
  | top => exact maximalCand
  | bvar i =>
    rw [interp]
  ...
  | fvar X => exact hρ.free_rc X
  | arrow σ τ ihσ ihτ =>
    have candσ : Candidate (interp ρ σ) := ihσ hρ
    have candτ : Candidate (interp ρ τ) := ihτ hρ
    refine ⟨?_, ?_, ?_, ?_, ?_⟩
    · exact fun ⟨lc, _⟩ => lc
    · exact fun ⟨_, sn, _⟩ => sn
    · intro t t' ⟨_, sn, hbody⟩ r
      ...
      intro s hs
      exact candτ.red (hbody s hs) (r.appₗ (candσ.lc hs))
    · intro t lc neu
    ...
    · intro t lc hnv hred
      ...
      refine ⟨lc, hsn, ?_⟩
      intro s hs
      refine candτ.headExpand (lc.app (candσ.lc hs)) nofun ?_
      intro u hu
      cases hu with
  ...
  | sum σ τ ihσ ihτ =>
    ...
    · intro t ht
      ...
      | red _ r ih => exact r.lc.right
      | headExpand lc hnv hred => exact lc
    · intro t ht
      induction ht with
      | inl hs => exact sn_inl (candσ.sn hs)
      | inr hs => exact sn_inr (candτ.sn hs)
      | neutral _ neu => exact sn_neutral neu
      | red hh r ih => exact ih.of_rel r
      | headExpand _ _ _ ih => exact .intro ih
    · exact SumInterp.red
    · exact SumInterp.neutral
    · exact SumInterp.headExpand
  | all σ τ ihσ ihτ =>
    refine ⟨?_, ?_, ?_, ?_, ?_⟩
    · exact fun ⟨lc, _⟩ => lc
    · exact fun ⟨_, sn, _⟩ => sn
    · intro t t' ⟨_, sn, hbody⟩ r
      ...
      exact (ihτ (hρ.extend_bound candS)).red (hbody S candS hsub U hU) (r.tapp hU)
    · intro t lc neu
      refine ⟨lc, sn_neutral neu, ?_⟩
      intro S candS hsub U hU
    ...
    · intro t lc hnv hred
      ...
      refine ⟨lc, hsn, ?_⟩
      intro S candS hsub U hU
      refine (ihτ (hρ.extend_bound candS)).headExpand (lc.tapp hU) nofun ?_
      intro u hu
      cases hu with
...
lemma interp_lc {ρ : Valuation Var} {τ : Ty Var} {t : Term Var}
    (hρ : ρ.IsValid) (mem : t ∈ interp ρ τ) : t.LC :=
  (interp_candidate τ hρ).lc mem

/-- Members of an interpretation are strongly normalizing. -/
lemma interp_sn {ρ : Valuation Var} {τ : Ty Var} {t : Term Var}
    (hρ : ρ.IsValid) (mem : t ∈ interp ρ τ) : SN Term.Red t :=
  (interp_candidate τ hρ).sn mem

omit [HasFresh Var] in
/-- Updating the free assignment at a variable absent from a type's free variables is inert. -/
lemma interp_free_update_nmem {ρ : Valuation Var} {τ : Ty Var} {X : Var} {C : Set (Term Var)}
    (nmem : X ∉ τ.fv) :
