...
/-- Weak head-expansion of an interpretation, via the candidate's `expand` field. -/
lemma interp_expand_gen {ρ : SemEnv Var} (hρ : ρ.IsValid) (δ : Ty Var) {t : Term Var}
    (lc : t.LC) (hnv : ¬ t.Value) (hred : ∀ t', t ⭢βᵛ t' → t' ∈ interp ρ δ) :
    t ∈ interp ρ δ :=
  (interp_cand δ hρ).expand lc hnv hred

/-- Term-β head-expansion for the function space. -/
lemma interp_expand_abs {ρ : SemEnv Var} {σ' : Ty Var} {σ τ : Ty Var} {M s : Term Var}
  ...
  have candσ : Cand (interp ρ σ) := interp_cand σ hρ
  ...
  | intro s _ ih =>
    intro sσ
    refine interp_expand_gen hρ τ (lc.app (candσ.lc sσ)) nofun ?_
    intro u hu
    cases hu with
    | appₗ _ r₁ => exact (value_no_red (.abs lc) r₁).elim
    | appᵣ _ r₂ => exact ih _ r₂ (candσ.fwd sσ r₂)
    | abs _ hv => exact hbody s sσ

/-- Type-β head-expansion for the universal. -/
lemma interp_expand_tabs {ρ : SemEnv Var} {σ' : Ty Var} {δ : Ty Var} {M : Term Var} {τ' : Ty Var}
    (hρ : ρ.IsValid) (lc : (Term.tabs σ' M).LC) (lcτ : τ'.LC)
    (hbody : (M ^ᵗᵞ τ') ∈ interp ρ δ) :
    Term.tapp (Term.tabs σ' M) τ' ∈ interp ρ δ := by
  refine interp_expand_gen hρ δ (lc.tapp lcτ) nofun ?_
  intro u hu
  cases hu with
...
lemma interp_expand_case_inl {ρ : SemEnv Var} {σ δ : Ty Var} {v N₂ N₃ : Term Var}
  ...
  refine interp_expand_gen hρ δ (Term.body_case.mpr ⟨val.lc.inl, b₂, b₃⟩) nofun ?_
  intro u hu
  cases hu with
...
lemma interp_expand_case_inr {ρ : SemEnv Var} {τ δ : Ty Var} {v N₂ N₃ : Term Var}
  ...
  refine interp_expand_gen hρ δ (Term.body_case.mpr ⟨val.lc.inr, b₂, b₃⟩) nofun ?_
  intro u hu
  cases hu with
...
lemma interp_openRec_fvar {ρ : SemEnv Var} {τ : Ty Var} {X : Var} {S : Set (Term Var)} {k : ℕ}
  ...
  | fvar Y =>
    rw [interp]
    dsimp only
    exact (Function.update_of_ne (a := Y) (a' := X) (by grind) S ρ.free).symm
  | arrow σ τ ihσ ihτ =>
    have ⟨hσ, hτ⟩ := hlc
  ...
  | all σ τ ihσ ihτ =>
    ...
    have key : ∀ T : Set (Term Var),
      ...
      intro T
      have := ihτ (ρ := { ρ with bound := T :: ρ.bound }) (by grind)
        (by simp only [List.length_cons]; omega) hτ
...
lemma interp_openTy_fvar {ρ : SemEnv Var} {τ : Ty Var} {X : Var} {S : Set (Term Var)}
...
lemma interp_bound_irr_at {ρ : SemEnv Var} {τ : Ty Var} {S : Set (Term Var)} {k : ℕ}
  ...
  | all σ τ ihσ ihτ =>
    ...
    have key : ∀ T : Set (Term Var),
      ...
      intro T
      have := ihτ (ρ := { ρ with bound := T :: ρ.bound }) (by simp only [List.length_cons]; omega)
        hτ
      simpa only [List.insertIdx_succ_cons] using this
    simp only [key]
...
lemma interp_bound_irr {ρ : SemEnv Var} {τ : Ty Var} {S : Set (Term Var)} (hlc : τ.LcAt 0) :
    interp { ρ with bound := S :: ρ.bound } τ = interp ρ τ := by
  have h := interp_bound_irr_at (ρ := ρ) (S := S) zero_le hlc
  rwa [List.insertIdx_zero] at h

omit [DecidableEq Var] [HasFresh Var] in
/-- Interpretation commutes with opening at depth `k` (semantic substitution at depth). -/
lemma interp_openRec_bound {ρ : SemEnv Var} {τ U : Ty Var} {k : ℕ}
  ...
  | bvar idx =>
    ...
    rw [Ty.openRec]
    rcases lt_trichotomy idx k with hlt | heq | hgt
    · rw [if_neg (by omega), interp, interp]
      rw [List.getElem?_insertIdx, if_pos hlt]
    · rw [heq, if_pos rfl, interp]
      rw [List.getElem?_insertIdx, if_neg (by omega), if_pos rfl, if_pos hk, Option.getD_some]
    · omega
  ...
  | all σ τ ihσ ihτ =>
    ...
    have key : ∀ T : Set (Term Var),
      ...
      intro T
      have h := ihτ (ρ := { ρ with bound := T :: ρ.bound }) (k := k + 1)
        (by simp only [List.length_cons]; omega) hτ
      rw [interp_bound_irr (ρ := ρ) (S := T) hlcU] at h
...
lemma interp_openTy_bound {ρ : SemEnv Var} {τ U : Ty Var} (hlcτ : τ.LcAt 1) (hlcU : U.LcAt 0) :
    interp { ρ with bound := interp ρ U :: ρ.bound } τ = interp ρ (τ ^ᵞ U) := by
  have := interp_openRec_bound (ρ := ρ) (U := U) zero_le hlcτ hlcU
  simpa only [Ty.open', List.insertIdx_zero] using this
...
def EnvCoh (Γ : Env Var) (ρ : SemEnv Var) : Prop :=
  ∀ {X σ}, Binding.sub σ ∈ Γ.dlookup X → ρ.free X ⊆ interp ρ σ

omit [HasFresh Var] in
/-- The extended environment updating a fresh free variable is valid. -/
lemma isValid_free_update {ρ : SemEnv Var} {X : Var} {S : Set (Term Var)} (hρ : ρ.IsValid)
    (candS : Cand S) : ({ ρ with free := Function.update ρ.free X S } : SemEnv Var).IsValid := by
...
lemma envCoh_free_update {Γ : Env Var} {ρ : SemEnv Var} {X : Var} {σ : Ty Var}
    {S : Set (Term Var)} (cohρ : EnvCoh Γ ρ) (hΓwf : Env.Wf Γ) (hXΓ : X ∉ Context.dom Γ)
    (hXσ : X ∉ σ.fv) (hsub : S ⊆ interp ρ σ) :
    EnvCoh (⟨X, Binding.sub σ⟩ :: Γ) ({ ρ with free := Function.update ρ.free X S }) := by
  intro Y σ'' bind
  by_cases hYX : Y = X
  · subst hYX
  ...
  · rw [List.dlookup_cons_ne (a := Y) Γ ⟨X, Binding.sub σ⟩ hYX] at bind
    ...
    have hfreeY : ({ ρ with free := Function.update ρ.free X S } : SemEnv Var).free Y = ρ.free Y :=
...
lemma interp_sub_mono {Γ : Env Var} {σ τ : Ty Var} (sub : Sub Γ σ τ)
    {ρ : SemEnv Var} (hρ : ρ.IsValid) (cohρ : EnvCoh Γ ρ) :
  ...
  | trans_tvar bind _ ih =>
    have hX : interp ρ (Ty.fvar _) ⊆ interp ρ _ := cohρ bind
    exact fun t ht => ih hρ cohρ (hX ht)
  | arrow _ _ ihσ ihτ =>
    intro t ⟨lc, sn, hbody⟩
    refine ⟨lc, sn, ?_⟩
    intro s hs
  ...
  | @all Γ σ σ' τ' τ L subσ subτ ihσ ihτ =>
    ...
    set ρ' : SemEnv Var := { ρ with free := Function.update ρ.free X S }
    have hρ'valid : ρ'.IsValid := isValid_free_update hρ candS
    have hcoh' : EnvCoh (⟨X, Binding.sub σ⟩ :: Γ) ρ' :=
      envCoh_free_update cohρ wfΓ (by grind) (by grind) hsub
  ...
  | sum _ _ ihσ ihτ =>
    ...
    | fwd _ r ih => exact SumInterp.fwd ih r
    | expand lc hnv _ ih => exact SumInterp.expand lc hnv ih
